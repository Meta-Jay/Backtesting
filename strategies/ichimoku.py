...
def backtest(df: pd.DataFrame, tenkan_period: int, kijun_period: int):
    # Tenkan period: short-term signal line
    ...
    # Kijun period: short-term signal line
    ...
    df["rolling_min_senkou"] = df["low"].rolling(window=2*kijun_period).min()
    df["rolling_max_senkou"] = df["high"].rolling(window=2*kijun_period).max()
